export const projects = [
    {
        id: 1,
        title: "Mama Mango's",
        slug: "mamamangos",
        category: "Branding & Web Design",
        description: "More Than Just A Bakery",
        year: "2024",
        role: "Brand Identity, Web Design",
        client: "Mama Mango's Bakery",
        image: "/thumbnails/mamamangos.png",
        link: "/work/mamamangos",
        details: {
            content: [], // Kept for compatibility, but empty as we use sections
            gallery: []  // Kept for compatibility
        },
        hideHero: true,
        sections: [
            {
                type: 'text',
                title: 'Overview',
                content: "Mama Mango’s Bakery is a capstone project close to my heart. It is a collaboration between my significant other, Nick Mangubat, and me, created in honor of his late mother, Donna Mangubat. As his biggest supporter, she encouraged his dreams and once told him that his croissants were better than any bakery’s. That simple, heartfelt praise became the spark for something bigger. Together, we are turning her words into a warm, welcoming brand where her memory lives on through his craft and my design."
            },
            {
                type: 'grid-2',
                items: [
                    { title: 'Service', content: 'Web Design, Package Design, UI/UX Design' },
                    { title: 'Software', content: 'Webflow, Adobe Illustrator, Adobe Photoshop, Adobe Premier Pro' }
                ]
            },
            {
                type: 'grid-2',
                items: [
                    { title: 'Problem', content: 'Many small bakeries struggle to stand out in a crowded market due to a lack of tools to share their unique stories and connect emotionally with customers, often missing out on the benefits of a strong online presence.' },
                    { title: 'Solution', content: 'We are designing an identity for the bakery and a website that goes beyond just selling pastries, it tells a story, creates an emotional connection, and provides a seamless customer experience.' }
                ]
            },
            {
                type: 'image-full',
                src: '/work/mamamangos/screencapture-about.png',
                alt: 'Mama Mangos About Page'
            },
            {
                type: 'image-full',
                src: '/work/mamamangos/iphone-blue.png',
                alt: 'Mobile View'
            },
            {
                type: 'text',
                title: 'Research & Data',
                content: "Rooted in warmth and simplicity, Mama Mango’s branding blends soft orange tones, cozy tans, and rich browns to create a space that feels both modern and nurturing. The clean, sleek design is elevated by subtle, earthy warmth, inviting guests into an environment that’s as comforting as a homemade pastry. This warm palette and minimal aesthetic reflect the heart of Mama Mango’s: a bakery where tradition meets thoughtful design."
            },
            {
                type: 'image-grid',
                images: [
                    '/work/mamamangos/mamamangos-logo.png',
                    '/work/mamamangos/img-6837.jpg',
                    '/work/mamamangos/resize-05.png'
                ]
            },
            {
                type: 'text',
                title: 'Brand Identity',
                content: "Designing a brand identity for Mama Mango’s involves crafting a cohesive visual language that connects seamlessly across the website, social media, packaging, and menu. Rooted in warmth and simplicity, the brand blends nostalgic charm with a clean, modern aesthetic, inviting customers into a heartfelt experience whether they’re browsing online or unwrapping a fresh pastry. The goal is to create a space that feels both personal and polished, appealing to a community that values comfort, quality, and thoughtful design."
            },
            {
                type: 'image-full',
                src: '/work/mamamangos/bakery-interior.jpg',
                alt: 'Bakery Interior'
            },
            {
                type: 'text',
                title: 'Logo Design',
                content: "The Mama Mango’s logo was inspired by the handwriting of Mama Donna, the heart behind the bakery. Her original script was traced and carefully refined to maintain its natural charm while enhancing readability and versatility. The result is a logo that feels personal and warm, capturing Donna’s presence in every curve and stroke, and preserving the authenticity at the core of the brand."
            },
            {
                type: 'image-grid',
                images: [
                    '/work/mamamangos/screencapture-contact.png',
                    '/work/mamamangos/untitled-3.png'
                ]
            },
            {
                type: 'text',
                title: 'Social Media',
                content: "Social media is an essential part of Mama Mango’s, offering a window into the heart of the bakery. Rather than polished, curated posts, the content highlights raw, unfiltered moments, flour-dusted counters, golden pastries fresh from the oven, and behind-the-scenes baking shots. This approach creates a friendly, approachable feel that invites customers to connect with the process, the people, and the warmth behind every treat."
            },
            {
                type: 'image-full',
                src: '/work/mamamangos/social-media-mockup.png',
                alt: 'Social Media Mockup'
            },
            {
                type: 'text',
                title: 'Package Design',
                content: "The product and print designs are crafted to reflect the bakery’s warm, homey character. Each piece features delicate pastry line work and incorporates the brand’s signature orange and tan tones, creating a sense of consistency and comfort. The main logo is prominently displayed on the front, tying every design element back to the heart of the brand and enhancing the inviting, handcrafted feel of the overall identity."
            },
            {
                type: 'image-grid',
                images: [
                    '/work/mamamangos/package-design.jpg',
                    '/work/mamamangos/group-14.png'
                ]
            },
            {
                type: 'text',
                title: 'Photography',
                content: "I took the photography for the bakery website to showcase the authentic, handcrafted look of the baked goods Nick baked and create a genuine connection with customers. To enhance their visual appeal, I edited the photos to align with the brand’s bright orange theme, making the treats look even more inviting and delicious. This combination of authenticity and vibrant color helps create an atmosphere that encourages customers to crave what they see."
            },
            {
                type: 'image-grid',
                images: [
                    '/work/mamamangos/recipes-1.jpg',
                    '/work/mamamangos/upscale-media.jpg',
                    '/work/mamamangos/gala.jpg',
                    '/work/mamamangos/resize-01.png',
                    '/work/mamamangos/resize-08.png',
                    '/work/mamamangos/resize-03.png',
                    '/work/mamamangos/brand.jpg',
                    '/work/mamamangos/resize-07.png',
                    '/work/mamamangos/resize-02.png'
                ]
            },
            {
                type: 'text',
                title: 'Mama Mango’s - Online',
                content: "We created a live website, MamaMangosBakery.com, to give the bakery a strong digital presence that reflects its warmth and story. The goal was to make it easy for customers to browse, connect, and shop, whether they’re discovering the bakery for the first time or returning for a favorite treat. The site is designed with clean navigation, clear categories for pastries, and a simple checkout process, so users can quickly find what they’re looking for and place an order without confusion. It balances storytelling with function, making the online experience feel as welcoming and thoughtful as the bakery itself."
            },
            {
                type: 'image-full',
                src: '/work/mamamangos/screencapture-home.png',
                alt: 'Website Home'
            },
            {
                type: 'text',
                title: 'Takeaways',
                content: "Mama Mango’s is more than a bakery, it’s a tribute, a memory, and a shared dream brought to life through design. Rooted in love and legacy, the brand honors a mother’s encouragement and transforms it into a warm, tangible space for others to experience. Through a thoughtfully crafted identity and story-driven website, Mama Mango’s bridges heartfelt tradition with modern connection. By focusing on emotional storytelling, accessible design, and an inviting visual presence, the project shows how small bakeries can stand out not just by what they sell but by what they represent. In every pastry, post, and page, Mama Mango’s keeps her spirit alive and welcomes others into something truly personal."
            }
        ]
    },
    {
        id: 2,
        title: "LifeLog",
        slug: "lifelog",
        category: "App Design",
        description: "Turn Daily Snaps Into Lifelong Memories",
        year: "2023",
        role: "UI/UX Design",
        client: "LifeLog Inc.",
        image: "/thumbnails/lifelog.png",
        link: "/work/lifelog",
        details: {
            content: [
                "LifeLog is a mobile application designed to help users document their daily lives through photos and short notes. The challenge was to create an interface that felt unobtrusive and encouraged daily habit-building.",
                "We utilized a clean, card-based layout and intuitive gestures to make logging entries quick and enjoyable."
            ],
            gallery: [
                "https://placehold.co/800x600/EEE/31343C?text=LifeLog+UI+1",
                "https://placehold.co/800x600/EEE/31343C?text=LifeLog+UI+2"
            ]
        }
    },
    {
        id: 3,
        title: "ODU Rec & Wellness",
        slug: "odu-src-impact-report",
        category: "Impact Report",
        description: "Impact Report 2025",
        year: "2025",
        role: "Editorial Design",
        client: "Old Dominion University",
        image: "/thumbnails/Odu Rec Well.png",
        link: "/work/odu-src-impact-report",
        details: {
            content: [
                "The 2025 Impact Report for ODU Rec & Wellness highlights the achievements and growth of the department. The design system relies on bold typography and dynamic layouts to present data in an engaging way.",
                "Infographics and large-scale photography were used to break up text and keep the reader interested throughout the report."
            ],
            gallery: [
                "https://placehold.co/800x600/EEE/31343C?text=Report+Spread+1",
                "https://placehold.co/800x600/EEE/31343C?text=Report+Spread+2"
            ]
        }
    },
    {
        id: 4,
        title: "QuickWay",
        slug: "quickway",
        category: "UX/UI Design",
        description: "Say Goodbye to Bus Confusions",
        year: "2024",
        role: "Product Design",
        client: "City Transit",
        image: "/thumbnails/Quickway.png",
        link: "/work/quickway",
        details: {
            content: [
                "QuickWay aims to simplify public transit navigation. By focusing on real-time updates and clear route visualization, we reduced user confusion by 40% in initial testing.",
                "The app features a high-contrast mode for accessibility and offline maps for reliability."
            ],
            gallery: [
                "https://placehold.co/800x600/EEE/31343C?text=App+Map+View",
                "https://placehold.co/800x600/EEE/31343C?text=Route+Selection"
            ]
        }
    },
    {
        id: 5,
        title: "Graphic Design Dept",
        slug: "odu-gd-branding",
        category: "Branding",
        description: "Building Identity: Brand Guidelines",
        year: "2023",
        role: "Brand Strategy",
        client: "ODU Art Department",
        image: "/thumbnails/Project Seed.png",
        link: "/work/odu-gd-branding",
        details: {
            content: [
                "A comprehensive rebrand for the Graphic Design Department. The new identity reflects the creative and experimental nature of the program.",
                "Deliverables included a logo suite, style guide, and environmental graphics for the studio spaces."
            ],
            gallery: [
                "https://placehold.co/800x600/EEE/31343C?text=Brand+Guide",
                "https://placehold.co/800x600/EEE/31343C?text=Logo+Suite"
            ]
        }
    },
    {
        id: 6,
        title: "Utilitarian",
        slug: "utilitarian",
        category: "Fashion / Web",
        description: "Utilitarian",
        year: "2024",
        role: "Art Direction, Web Design",
        client: "Utilitarian Brand",
        image: "https://placehold.co/1200x800/EEE/31343C?text=Utilitarian+Cover",
        link: "/work/utilitarian",
        details: {
            content: [
                "Utilitarian is a fashion brand focused on functional aesthetics. The website mirrors this philosophy with a brutalist layout and raw interactions.",
                "We used large typography and sticky scrolling elements to create a unique browsing experience."
            ],
            gallery: [
                "https://placehold.co/800x600/EEE/31343C?text=Lookbook",
                "https://placehold.co/800x600/EEE/31343C?text=Product+Page"
            ]
        }
    },
    {
        id: 7,
        title: "UBE Cold Brew",
        slug: "ube-cold-brew",
        category: "Web Design",
        description: "From Bean to Screen",
        year: "2024",
        role: "Web Design",
        client: "UBE Coffee Co.",
        image: "https://placehold.co/1200x800/EEE/31343C?text=UBE+Cold+Brew+Cover",
        link: "/work/ube-cold-brew",
        details: {
            content: [
                "A promotional landing page for a new line of Ube-infused cold brew coffee. The site features parallax scrolling and vibrant purple hues to match the product.",
                "The goal was to drive pre-orders, which exceeded expectations by 200%."
            ],
            gallery: [
                "https://placehold.co/800x600/EEE/31343C?text=Landing+Page",
                "https://placehold.co/800x600/EEE/31343C?text=Ingredients+Section"
            ]
        }
    },
    {
        id: 8,
        title: "Shiori",
        slug: "shiori",
        category: "App Design",
        description: "The Art of Simplicity",
        year: "2023",
        role: "UI Design",
        client: "Shiori Reading App",
        image: "/thumbnails/shiori.png",
        link: "/work/shiori",
        details: {
            content: [
                "Shiori is a minimalist bookmarking app. The design focuses on typography and whitespace to create a calm environment for organizing reading lists.",
                "Dark mode was a priority, ensuring a comfortable experience for night-time users."
            ],
            gallery: [
                "https://placehold.co/800x600/EEE/31343C?text=Dark+Mode",
                "https://placehold.co/800x600/EEE/31343C?text=List+View"
            ]
        }
    },
    {
        id: 9,
        title: "Vibeify",
        slug: "vibeify",
        category: "App Design",
        description: "Your City, Your Sound",
        year: "2024",
        role: "Product Design",
        client: "Vibeify Startup",
        image: "https://placehold.co/1200x800/EEE/31343C?text=Vibeify+Cover",
        link: "/work/vibeify",
        details: {
            content: [
                "Vibeify connects users with local music events based on their listening history. The interface is vibrant and energetic, using album art colors to theme the UI dynamically.",
                "We focused on a 'swipe' interaction model for discovering new events nearby."
            ],
            gallery: [
                "https://placehold.co/800x600/EEE/31343C?text=Event+Discovery",
                "https://placehold.co/800x600/EEE/31343C?text=UserProfile"
            ]
        }
    }
];
